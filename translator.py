...
# Poster ratings
RATINGS_SERVER = os.getenv("TR_SERVER", "https://api.ratingposterdb.com")
...
def translate_catalog(original: dict, tmdb_meta: dict, skip_poster, toast_ratings) -> dict:
    ...
    for i, item in enumerate(new_catalog["metas"]):
        try:
            ...
            # Set poster if content doesn't have tmdb information
            if toast_ratings == "1":
                if "tt" in tmdb_meta[i].get("imdb_id", ""):
                    item["poster"] = (
                        f"https://api.ratingposterdb.com/t0-free-rpdb/imdb/poster-default/{tmdb_meta[i]['imdb_id']}.jpg"
                    )
